...
#			a Vector Compare Equal-To instruction where the syntax is vec_cmpeq d, a, b. Vector a, b, and d
#			consist of eight 8-bit elements. When the instruction is execute, each element of the result vector
#			d is TRUE (all bits = 1) if the corresponding element of vector a is equal to the corresponding
#			element of vector b. Otherwise, the element of result is FALSE (all bits = 0)
#
...
loop:		and	$t1, $s0, $t0		# load 8 bits of $s0 (an element of a) into $t1
		...
		bne	$t1, $t2, notEqual1	# jump to notEqual1 if they are not equal
		
equal1:		add	$s4, $s4, $t0		# load 8 bits in corresponding section of $s2 (an element of b) into $t2
		
notEqual1:	and	$t1, $s1, $t0		# load 8 bits of $s0 (an element of a) into $t1
		and	$t2, $s3, $t0		# load 8 bits in corresponding section of $s3 (an element of b) into $t2
		bne	$t1, $t2, notEqual2	# jump to notEqual2 if they are not equal
		
equal2:		add	$s5, $s5, $t0		# load 8 bits in corresponding section of $s2 (an element of b) into $t2

notEqual2:	sll	$t0, $t0, 8		# adjust the pointer $t0 to the next element
