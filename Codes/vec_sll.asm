...
#			a Vector Shift Left Logically instruction where the syntax is vec_sll d, a, b.
#			Vector a and d consist of eight 8-bit elements. When the instruction is executed, vector a is
#			shifted left logically by the number of elements stored in b.
#
...
lower:		addi	$t1, $t1, -32			# calculate the number of bits to be shifted in lower 32-bit section
		sllv	$s4, $s0, $t1			# shift left logically the lower 32-bit section of vector a
		j	clearReg
		
		# If the number of bits to be shifted is less than 32
upper:		sllv	$s3, $s0, $t1			# shift left the upper 32-bit section of vector a
							#   and store the result in the upper 32-bit section of vector d
		srlv	$t2, $s1, $t1			# take the bits of the lower section that are truncated after being shifted
		sllv	$s4, $s1, $t1			# shift left the lower 32-bit section of vector a by same number of bits
							#   that is shifted in the upper section to leave space for the missed bits
		add	$s3, $s3, $t2			# copy the missed bits to the upper 32-bit section of vector d
